...
graph LR
    ...
    subgraph Hệ thống SCIM
        %% Use cases
        UC_Auth(Xác thực & Profile)
        UC_User(Quản lý Thành viên & Phân quyền)
        UC_Settings(Cấu hình hệ thống & Audit)
        UC_Master(Quản lý Danh mục - Master Data)
        UC_Inbound(Quản lý Nhập kho)
        UC_Outbound(Quản lý Xuất kho)
        UC_Stocktake(Kiểm kê & Điều chỉnh)
        UC_QR(In & Quét mã QR)
        UC_Report(Xem Dashboard & Báo cáo)
    end
    ...
    %% Relations for Viewer
    Viewer --> UC_Auth
    Viewer --> UC_Report

    %% Style styling
    classDef actor fill:#e1f5fe,stroke:#01579b,stroke-width:2px,color:#01579b;
    classDef usecase fill:#e8f5e9,stroke:#2e7d32,stroke-width:1px,color:#1b5e20;
    class Admin,Manager,Staff,Viewer actor;
    class UC_Auth,UC_User,UC_Settings,UC_Master,UC_Inbound,UC_Outbound,UC_Stocktake,UC_QR,UC_Report usecase;
```
...
graph TB
    ...
    subgraph Phân hệ Nhập / Xuất kho
        UC_CreateIn(Lập phiếu Nhập kho)
        UC_ApproveIn(Duyệt phiếu Nhập kho)
        UC_WAC(Tính giá vốn tự động)
        
        UC_CreateOut(Lập phiếu Xuất kho)
        UC_SuggestOut(Gợi ý lô hàng & Vị trí FEFO/FIFO)
        UC_ApproveOut(Duyệt & Đóng gói phiếu Xuất)

        UC_ApproveIn -.->|include| UC_WAC
        UC_CreateOut -.->|include| UC_SuggestOut
    end

    Staff --> UC_CreateIn
    Staff --> UC_CreateOut
    
    Manager --> UC_ApproveIn
    Manager --> UC_ApproveOut

    classDef actor fill:#e1f5fe,stroke:#01579b,stroke-width:2px,color:#01579b;
    classDef usecase fill:#fff3e0,stroke:#e65100,stroke-width:1px,color:#e65100;
    class Staff,Manager actor;
    class UC_CreateIn,UC_ApproveIn,UC_WAC,UC_CreateOut,UC_SuggestOut,UC_ApproveOut usecase;
```
...
graph LR
    ...
    subgraph Phân hệ Kiểm kê kho
        UC_CreateSession(Tạo phiên Kiểm kê)
        UC_Count(Ghi nhận số lượng thực đếm)
        UC_ScanQR(Quét mã QR sản phẩm/vị trí)
        UC_Diff(Tính chênh lệch tự động)
        UC_Adjust(Duyệt phiếu Điều chỉnh kho)

        UC_Count -.->|extend| UC_ScanQR
        UC_Count -.->|include| UC_Diff
    end
    ...
    classDef actor fill:#e1f5fe,stroke:#01579b,stroke-width:2px,color:#01579b;
    classDef usecase fill:#f3e5f5,stroke:#4a148c,stroke-width:1px,color:#4a148c;
    class Staff,Manager actor;
    class UC_CreateSession,UC_Count,UC_ScanQR,UC_Diff,UC_Adjust usecase;
```
...
### 2. Thuộc tính động của sản phẩm (JSONB Attributes Mapping)
Để đáp ứng yêu cầu quản lý đa dạng mặt hàng (linh kiện điện tử, thực phẩm có nhiệt độ bảo quản, hóa chất có chỉ số độc hại...), bảng `products` thiết kế trường `attributes` kiểu dữ liệu `jsonb`. Ở phía backend, trường này được map với một Map trong Java:
```java
@Column(name = "attributes", columnDefinition = "jsonb")
@Type(JsonBinaryType.class) // Sử dụng Hypersistence Optimizer để map JSONB
private Map<String, Object> attributes;
```
Giúp lưu trữ động các cặp Key-Value mà không cần thay đổi cấu trúc bảng dữ liệu vật lý.
...
```mermaid
graph TD
    Start([Bắt đầu hoàn thành phiếu nhập]) --> GetItem[Lấy chi tiết sản phẩm, số lượng nhập Q_new, đơn giá P_new]
    GetItem --> GetTotalStock[Truy vấn tổng tồn kho hiện tại Q_total từ StockLevel]
    GetTotalStock --> CalcOldStock[Tính tồn kho trước nhập: Q_old = Q_total - Q_new]
    CalcOldStock --> CheckStock{Q_old > 0?}
    
    CheckStock -- Có --> CalcValue[Tính giá trị cũ: V_old = Q_old * Product.price]
    CalcValue --> CalcNewWac[Tính WAC: Wac_new = (V_old + Q_new * P_new) / Q_total]
    CalcNewWac --> UpdateProduct[Cập nhật Product.price = Wac_new]
    
    CheckStock -- Không --> DirectWac[Cập nhật Product.price = P_new]
    DirectWac --> UpdateProduct
    
    UpdateProduct --> UpdateStockLevel[Cập nhật số dư StockLevel và tạo StockMovement]
    UpdateStockLevel --> End([Hoàn thành])
```

#### Code hiện thực thực tế (Trích xuất từ `InventoryTransactionServiceImpl.java`):
```java
// Lấy tổng tồn kho hiện tại (đã bao gồm lượng vừa cộng thêm trong transaction)
BigDecimal totalStock = stockLevelRepository.getTotalStockByProduct(product.getId()).orElse(BigDecimal.ZERO);
BigDecimal currentPrice = product.getPrice();
BigDecimal oldStock = totalStock.subtract(item.getQuantity()); // Tồn kho trước nhập
...
if (oldStock.compareTo(BigDecimal.ZERO) > 0) {
    ...
    product.setPrice(item.getPrice()); // Nếu trước đó không có tồn, lấy giá nhập mới làm giá vốn
}
...
```mermaid
graph TD
    Start([Nhận yêu cầu xuất sản phẩm X, số lượng Q_request]) --> QueryStock[Truy vấn tồn kho chi tiết có sắp xếp theo ExpirationDate tăng dần - FEFO]
    QueryStock --> LoopStock{Duyệt từng bản ghi tồn kho StockLevel}
    
    LoopStock -->|Hết danh sách hoặc Q_request = 0| CheckDone{Q_request = 0?}
    LoopStock -->|Còn bản ghi| CalcQty[Lấy lượng khả dụng khả thi: Q_avail = quantity - reservedQuantity]
    CalcQty --> CheckAvail{Q_avail > 0?}
    
    CheckAvail -- Có --> TakeQty[Lấy lượng xuất: Q_take = min Q_avail, Q_request]
    TakeQty --> AddSuggest[Thêm lô hàng & vị trí vào Picking List]
    AddSuggest --> DeductRequest[Trừ lượng cần xuất: Q_request = Q_request - Q_take]
    DeductRequest --> LoopStock
    
    CheckAvail -- Không --> LoopStock
    
    CheckDone -- Đúng --> SavePicking[Lưu Picking List gợi ý]
    CheckDone -- Sai --> ErrorStock[Báo lỗi: Tồn kho khả dụng không đủ đáp ứng]
```

#### Code truy vấn JPQL FEFO trong Repository (`StockLevelRepository.java`):
```java
@Query("SELECT s FROM StockLevel s JOIN s.batch b " +
       "WHERE s.product.id = :productId AND s.warehouse.id = :warehouseId " +
       "AND (s.quantity - s.reservedQuantity) > 0 " +
       "ORDER BY b.expiryDate ASC, s.createdAt ASC")
List<StockLevel> findAvailableStockFEFO(Long productId, Long warehouseId);
```

---

### 3. Cơ chế Khóa phân tán (Redis Lock) tránh xung đột xuất âm kho
Khi xảy ra tình huống có nhiều nhân viên kho cùng bấm duyệt xuất một mặt hàng tại cùng một thời điểm, nếu không được kiểm soát sẽ dẫn đến việc cả hai giao dịch cùng đọc một số dư tồn kho giống nhau và thực hiện trừ tồn kho, dẫn đến số dư thực tế bị âm (Race Condition). 

SCIM giải quyết triệt để vấn đề này bằng cách sử dụng **Khóa phân tán Redis** dựa trên `StringRedisTemplate` kết hợp với cấu trúc Transaction của Spring để cô lập luồng ghi dữ liệu.

```java
// Sắp xếp danh sách ID sản phẩm tăng dần để tránh Deadlock khi khóa nhiều sản phẩm
...
try {
    for (Long pId : productIds) {
        ...
        // Thực hiện SETNX với TTL = 15 giây
    ...
    // Thực thi nghiệp vụ thay đổi tồn kho thực tế...
    executeOutbound(transaction);

} finally {
    // Giải phóng tất cả các khóa sau khi hoàn tất giao dịch (hoặc khi gặp lỗi)
...
Tất cả các logic nghiệp vụ quan trọng ở Backend được viết Unit Test để tự động chạy kiểm chứng mỗi lần build mã nguồn (Continuous Integration - CI). Dự án sử dụng **JUnit 5** kết hợp với **Mockito** để cô lập các tầng kiểm thử.

#### Mã kiểm thử mẫu cho thuật toán tính giá vốn WAC (`InventoryTransactionServiceTest.java`):
...
import com.scim.service.impl.InventoryTransactionServiceImpl;
...
public class InventoryTransactionServiceTest {
    ...
    void setUp() {
        testProduct = Product.builder()
                ...
                .price(new BigDecimal("15000.0000")) // Giá vốn hiện tại: 15k
                .isActive(true)
                .build();
    ...
    @Test
    void testCalculateWacWhenCompletingInbound() {
        // Giả sử sau khi cộng dồn lượng nhập mới là 100 hộp, tổng tồn kho thực tế hiện tại là 200 hộp
        // Như vậy: Số lượng cũ = 100 hộp, Giá vốn cũ = 15k
        // Lô nhập mới: Số lượng = 100 hộp, Giá nhập mới = 17k
        // Công thức WAC mới mong đợi: (100 * 15k + 100 * 17k) / 200 = 16k
        
        BigDecimal newQty = new BigDecimal("100");
        BigDecimal newPrice = new BigDecimal("17000.0000");

        when(stockLevelRepository.getTotalStockByProduct(1L)).thenReturn(Optional.of(new BigDecimal("200")));
        
        // Thực thi logic tính toán WAC giả lập
        BigDecimal totalStock = stockLevelRepository.getTotalStockByProduct(1L).orElse(BigDecimal.ZERO);
        BigDecimal currentPrice = testProduct.getPrice();
        BigDecimal oldStock = totalStock.subtract(newQty);

        BigDecimal newWac;
        if (oldStock.compareTo(BigDecimal.ZERO) > 0) {
            BigDecimal oldTotalValue = oldStock.multiply(currentPrice);
            BigDecimal newTotalValue = oldTotalValue.add(newQty.multiply(newPrice));
            newWac = newTotalValue.divide(totalStock, 4, java.math.RoundingMode.HALF_UP);
            testProduct.setPrice(newWac);
        } else {
            testProduct.setPrice(newPrice);
        }

        // Kiểm tra kết quả
        assertEquals(new BigDecimal("16000.0000"), testProduct.getPrice());
    }
}
```
